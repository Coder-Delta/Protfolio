    ...
    <main class="analytics-page">
        ...
        <section v-else class="dashboard">
            ...
            <template v-if="data && !loading">
                ...
                <div class="charts"><article class="wide"><h2>Daily sessions</h2><div class="trend"><div v-for="item in data.trends" :key="item.label" class="bar-item"><span class="bar" :style="{ height: `${(item.value / max(data.trends)) * 100}%` }" :title="`${item.label}: ${item.value}`"></span><small>{{ item.label.slice(5) }}</small></div></div></article><DataList title="Most visited pages" :items="data.pages" /><DataList title="Most viewed projects" :items="data.projects" /><DataList title="Traffic sources" :items="data.sources" /><DataList title="Locations" :items="data.locations" /><DataList title="Devices" :items="data.devices" /><DataList title="Browsers" :items="data.browsers" /><DataList title="Operating systems" :items="data.operatingSystems" /></div>
            </template>
        </section>
    </main>
</template>
